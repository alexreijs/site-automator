var task = {
	...
	"steps": [
		...
		},
		{
			...
			"steps": function() {		
				page.evaluate(function(metric){
					inputs = document.getElementsByClassName('x-combo-list-item');
					for (i in inputs) {
						input = inputs[i];
						if (input.innerHTML == metric)
							input.click();
					}
				}, systemArguments['metric']);
			}
		},
		{
...
//console.log(systemArguments['metric']);
//slimer.exit();

var fs = require('fs');
var taskInterval;
var address = 'http://360yield.com';
var credentials = require('credentials.js').getCredentials('360-yield');
